...
Program* Program::getProgram(std::string name) {
    if (program_cache.find(name) == program_cache.end()) {
        ...
        new_program.id_ = glCreateProgram();
        ...
        if (vertex_shader)
            glAttachShader(new_program.id_, vertex_shader);
        else
            std::cout << "[Program]\t Failed to load " << vertex_filename << std::endl;

        if (fragment_shader)
            glAttachShader(new_program.id_, fragment_shader);
        else
            std::cout << "[Program]\t Failed to load " << fragment_filename << std::endl;

        glLinkProgram(new_program.id_);

        if (vertex_shader) {
            glDetachShader(new_program.id_, vertex_shader);
            glDeleteShader(fragment_shader);
        }

        if (vertex_shader) {
            glDetachShader(new_program.id_, fragment_shader);
            glDeleteShader(fragment_shader);
        }
...
GLint Program::uniformLocation(std::string uniform) {
    std::map<std::string, GLint>::iterator uniform_search = uniform_locations.find(uniform);
    if (uniform_search != uniform_locations.end()) {
        return uniform_search->second;
    }
    else {
        GLint loc = glGetUniformLocation(id_, uniform.c_str());
        if (loc != -1) {
